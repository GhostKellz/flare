...
pub const Lexer = struct {
    ...
    fn scanToken(self: *Lexer) LexError!void {
        ...
        switch (c) {
            ...
            '.' => {
                ...
                if (self.current > 1 and std.ascii.isDigit(self.source[self.current - 2])) {
    ...
    fn identifier(self: *Lexer) LexError!void {
    ...
    fn string(self: *Lexer, quote: u8, literal: bool) LexError!void {
    ...
    fn number(self: *Lexer) LexError!void {
        var is_float = false;
        ...
        // Check for date-time pattern (YYYY-MM-DD)
        ...
        // Check for base-prefixed integers (0x, 0o, 0b)
        if (!self.isAtEnd() and self.peek() == '0' and self.current + 1 < self.source.len) {
            const prefix = self.source[self.current + 1];
    ...
    fn time(self: *Lexer) LexError!void {
    ...
    fn addToken(self: *Lexer, token_type: TokenType) LexError!void {
...
test "lexer basic tokens" {
    ...
    var lexer_inst = Lexer.init(testing.allocator, source);
    defer lexer_inst.deinit();

    const tokens = try lexer_inst.scanTokens();
